// File moved to ./Store/ChiTietTranh.jsx
...
const handleBuyNow = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-10">
      ...
      {/* Zoom ảnh */}
      {/* Zoom ảnh (thay thế phần Zoom hiện tại bằng phần này) */}
<AnimatePresence>
  {isZoomed && (
    <motion.div
    ...
    >
      {/* Backdrop: mờ & blur nền */}
      <div
        className="absolute inset-0 bg-black/70 backdrop-blur-sm"
        aria-hidden="true"
      />

      {/* Container ảnh: stopPropagation để click ngoài đóng modal vẫn hoạt động */}
      <motion.div
      ...
      >
        {/* 
          Dùng div với background-image thay vì <img>.
          overlayCapture: lớp che chắn phía trên để chặn chuột phải/drag.
        */}
        <div
        ...
        >
          {/* Hộp ảnh (background) */}
          <div
            // hiển thị ảnh watermark nếu có, nếu không dùng ảnh gốc
            style={{
              width: "100%",
              height: "100%",
              backgroundImage: `url(${watermarkedImage || image})`,
              backgroundPosition: "center",
              backgroundSize: "contain",
              backgroundRepeat: "no-repeat",
            }}
            // chặn chuột phải và kéo
            onContextMenu={(e) => e.preventDefault()}
            onDragStart={(e) => e.preventDefault()}
            onMouseDown={(e) => {
              // chặn mousedown phải chuột (nếu cần)
              if (e.button === 2) e.preventDefault();
            }}
            // CSS để ngăn kéo/selected
            className="select-none"
          />

          {/* Lớp overlay trong suốt chặn mọi tương tác với ảnh (nhưng cho phép bạn đặt watermark hiển thị) */}
          <div
            className="absolute inset-0"
            // chặn context menu, drag, copy
            onContextMenu={(e) => e.preventDefault()}
            onDragStart={(e) => e.preventDefault()}
            onMouseDown={(e) => {
              if (e.button === 2) e.preventDefault();
            }}
            // pointerEvents để overlay chặn tương tác với ảnh
            style={{ pointerEvents: "auto" }}
          >
            {/* Semi-transparent decorative watermark (vẫn hiển thị) */}
            <img
              src={logoWatermark}
              alt="watermark"
              className="absolute opacity-60 bottom-6 right-6 w-28 pointer-events-none select-none"
              // đảm bảo logo không bị kéo
              onDragStart={(e) => e.preventDefault()}
              draggable={false}
            />
